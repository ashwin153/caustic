...
package object runtime {
  ...
  // Exception Types.
  def ExecutionException(msg: String): thrift.ExecutionException with NonRetryable =
    new thrift.ExecutionException(msg) with NonRetryable
  def ConflictException(keys: Set[Key]): thrift.ConflictException =
    new thrift.ConflictException(keys.asJava)
