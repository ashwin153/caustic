...
package object syntax {
  ...
  // Literals Values.
  implicit def bol2flag(x: Boolean): thrift.Transaction = flag(x)
  implicit def num2flag[T](x: T)(implicit num: Numeric[T]): thrift.Transaction = real(num.toDouble(x))
  implicit def str2flag(x: String): thrift.Transaction = text(x)
