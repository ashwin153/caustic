...
package object runtime {
  ...
  implicit def proxy2txn(proxy: Proxy): Transaction = proxy.owner match {
    case None => proxy.key
  ...
  def abs(x: Transaction): Transaction = branch(less(x, Literal.Zero), sub(Zero, x), x)
  ...
  def ceil(x: Transaction): Transaction = add(floor(x), Literal.One)
  ...
  def exists(x: Transaction): Transaction = not(equal(read(x), Literal.Empty))
