...
import schema.runtime.syntax.Context
...
trait Database {
  ...
  def execute(txn: Transaction)(
    ...
    @tailrec def fold(
      ...
      case Nil =>
        if (operands.size > 1)
          throw FatalException(s"Transaction evaluates to $operands.")
        operands.head
      ...
      case Right(op) :: rest =>
        (op, operands) match {
          ...
          case _ => throw FatalException(s"$op cannot be applied to $operands.")
        }
    }
  ...
  /**
   * Asynchronously executes the transaction generated by the specified function.
   *
   * @param f Transaction generator.
   * @param ec Implicit execution context.
   * @return Result of transaction execution, or an exception on failure.
   */
  def execute(f: Context => Unit)(
    implicit ec: ExecutionContext
  ): Future[String] = {
